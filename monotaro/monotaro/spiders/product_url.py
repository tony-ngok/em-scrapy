...
class MonotaroProductUrl(scrapy.Spider):
    ...
    def start_requests(self):
        # self.start_urls = ['https://www.monotaro.com/jp/ja/store/cmdty/section/S1070204?web_store=web_section_lv3_search_by_category'] # test
        for i, cu in enumerate(self.start_urls, start=1):
            print(f"{i:_}".replace('_', '.'), cu)
            cat_no = self.get_cat_no(cu)
            
            yield scrapy.Request('https://www.monotaro.com/jp/ja/ec-bff/graphql', headers=self.get_headers(cu),
                                 meta={
                                         'cat_url': cu,
                                         'cat_no': cat_no,
                                         'page': 0
    ...
    def parse(self, response: HtmlResponse):
        ...
        for prod in produits:
            ...
            if prod_id not in self.prod_ids:
                ...
                yield {
                    "prod_url": 'https://www.monotaro.com/jp/ja'+prod['link']
                }
